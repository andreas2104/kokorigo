    ...
    <main className="min-h-screen bg-[#faf8f4] text-[#111d35] transition-colors dark:bg-[#080d18] dark:text-slate-100">
      ...
      <header className="sticky top-0 z-30 border-b border-[#eee4d8] bg-white/95 backdrop-blur dark:border-slate-800 dark:bg-[#0b1120]/95">
        <div className="mx-auto flex h-[72px] max-w-[1500px] items-center gap-3 px-4 lg:px-6">
          ...
          <div><p className="text-lg font-bold leading-5">Kokorigo</p><p className="text-xs text-[#768096] dark:text-slate-400">Bibliothèque EPUB</p></div>
          <button type="button" onClick={toggleTheme} className="ml-auto inline-flex h-10 items-center gap-2 rounded-full border border-[#f0d8bb] bg-[#fffaf2] px-3 text-sm font-medium text-[#9a5200] transition hover:bg-[#fff3df] dark:border-slate-700 dark:bg-slate-800 dark:text-amber-200 dark:hover:bg-slate-700" aria-label={isDarkMode ? "Activer le mode jour" : "Activer le mode nuit"} title={isDarkMode ? "Mode jour" : "Mode nuit"}>{isDarkMode ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}<span className="hidden sm:inline">{isDarkMode ? "Mode jour" : "Mode nuit"}</span></button>
      ...
      {error && <div role="alert" className="mx-auto mt-4 flex max-w-[1460px] items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/60 dark:text-red-200"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />{error}</div>}

      <div className="mx-auto grid max-w-[1500px] items-start gap-4 p-4 lg:grid-cols-[280px_minmax(0,1fr)_300px] lg:p-5">
        <aside className="flex min-h-[720px] flex-col rounded-2xl border border-[#e9e1d7] bg-white p-3.5 shadow-[0_8px_30px_rgba(59,40,12,.04)] dark:border-slate-800 dark:bg-slate-900">
          ...
          <label className="mt-3 flex h-11 items-center gap-2 rounded-xl border border-[#e6e2dc] bg-[#fcfbf9] px-3 text-[#89909d] dark:border-slate-700 dark:bg-slate-950 dark:text-slate-500"><Search className="h-4 w-4" /><span className="sr-only">Rechercher un livre</span><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm text-[#172038] outline-none dark:text-slate-100" placeholder="Rechercher un livre…" /></label>
          <div className="mt-4 flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto lg:max-h-[545px]">
            {booksQuery.isLoading && <div className="flex flex-1 items-center justify-center gap-2 text-sm text-[#768096] dark:text-slate-400"><Loader2 className="h-4 w-4 animate-spin" />Chargement…</div>}
            ...
            {!booksQuery.isLoading && !booksQuery.isError && !books.length && <div className="grid flex-1 place-items-center px-4 text-center text-sm leading-6 text-[#768096] dark:text-slate-400">{query ? "Aucun résultat." : "Aucun livre importé."}</div>}
            {books.map((book, index) => (
              <div key={book.id} className="group flex gap-3 rounded-xl border border-[#ece7df] bg-white p-2 transition hover:border-[#e9c895] hover:shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:hover:border-amber-700">
                <button type="button" onClick={() => void openBook(book)} disabled={busyBookId !== null} className="flex min-w-0 flex-1 gap-3 text-left disabled:cursor-wait">
                  ...
                  <span className="min-w-0 py-1"><strong className="line-clamp-2 text-sm leading-5">{book.title}</strong><span className="mt-1 block truncate text-xs text-[#768096] dark:text-slate-400">{book.author || "Auteur non renseigné"}</span><span className="mt-2 flex items-center gap-1 text-[11px] font-medium text-[#b65b00] dark:text-amber-400">{busyBookId === book.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <BookOpen className="h-3 w-3" />} Lire</span></span>
                </button>
        ...
        {file ? (
          <div ref={readerSectionRef} className="min-w-0 scroll-mt-24">
          ...
          <section onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void importFile(event.dataTransfer.files?.[0]); }} className={`grid min-h-[720px] place-items-center rounded-2xl border-2 border-dashed bg-white px-6 text-center shadow-[0_8px_30px_rgba(59,40,12,.04)] transition dark:bg-slate-900 ${dragging ? "border-[#c96b00] bg-[#fff9ef] dark:bg-amber-950/20" : "border-[#ecc995] dark:border-slate-700"}`}>
            <div className="max-w-xl">
              ...
              <p className="mx-auto mt-3 max-w-lg leading-7 text-[#768096] dark:text-slate-400">Ou parcourez vos dossiers. Le livre est enregistré dans votre bibliothèque locale puis ouvert dans ce lecteur.</p>
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3 text-sm"><span className="flex items-center gap-2 rounded-full border border-[#e9e1d7] px-4 py-2 text-[#59647a] dark:border-slate-700 dark:text-slate-300"><ShieldCheck className="h-4 w-4 text-emerald-600" /> Traitement local</span><span className="flex items-center gap-2 rounded-full border border-[#e9e1d7] px-4 py-2 text-[#59647a] dark:border-slate-700 dark:text-slate-300"><Headphones className="h-4 w-4 text-[#c56500]" /> Piper + Kokoro + F5-TTS</span></div>
            </div>
          </section>
        )}

        <aside className="min-h-[720px] rounded-2xl border border-[#e9e1d7] bg-white p-3.5 shadow-[0_8px_30px_rgba(59,40,12,.04)] dark:border-slate-800 dark:bg-slate-900">
          <div className="px-1 py-2"><h2 className="flex items-center gap-3 text-xl font-bold"><AudioLines className="h-5 w-5 text-[#c66600]" /> Lecture vocale</h2><p className="mt-1 text-sm text-[#768096] dark:text-slate-400">Voix et rythme de lecture</p></div>

          <section className="mt-4 rounded-xl border border-[#ece5dc] p-3 dark:border-slate-800"><p className="flex items-center gap-2 text-sm font-semibold"><Volume2 className="h-4 w-4 text-[#c66600]" /> Voix sélectionnée</p><label className="relative mt-3 block"><span className="sr-only">Voix</span><select value={selectedVoice?.id ?? ""} onChange={(event) => setVoiceId(event.target.value)} disabled={voicesQuery.isLoading || !voices.length} className="h-14 w-full appearance-none rounded-xl border border-[#e8e2da] bg-[#fffdfa] px-3 pr-9 text-sm font-semibold outline-none focus:border-[#d27a0b] dark:border-slate-700 dark:bg-slate-950"><option value="">{voicesQuery.isLoading ? "Chargement des voix…" : "Aucune voix disponible"}</option>{voices.map((voice) => <option key={voice.id} value={voice.id}>{voice.name} · {voice.engine === "f5tts" ? "F5-TTS" : voice.engine === "kokoro" ? "Kokoro" : "Piper"}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-5 h-4 w-4 text-[#778093]" /></label>{selectedVoice && <p className="mt-2 text-xs text-[#768096] dark:text-slate-400">{selectedVoice.language} · {selectedVoice.character || "Voix naturelle"}</p>}</section>

          <section className="mt-3 rounded-xl border border-[#ece5dc] p-3 dark:border-slate-800"><p className="flex items-center gap-2 text-sm font-semibold"><Gauge className="h-4 w-4 text-[#c66600]" /> Vitesse de lecture</p><p className="mt-3 text-sm">{speed.toFixed(1)}× {speed === 1 ? "(Normale)" : ""}</p><input type="range" min="0.8" max="1.5" step="0.1" value={speed} onChange={(event) => setSpeed(Number(event.target.value))} className="mt-3 w-full accent-[#c66600]" /><div className="flex justify-between text-xs text-[#8a91a0] dark:text-slate-500"><span>Plus lent</span><span>Plus rapide</span></div><div className="mt-3 grid grid-cols-4 gap-1">{speeds.map((value) => <button key={value} onClick={() => setSpeed(value)} className={`rounded-lg py-1.5 text-xs ${speed === value ? "bg-[#fff0d9] font-semibold text-[#a65000] dark:bg-amber-950 dark:text-amber-300" : "bg-[#f7f5f2] text-[#687185] dark:bg-slate-800 dark:text-slate-300"}`}>{value}×</button>)}</div></section>
