...
{
    private readonly IPiperTtsService _tts;

    public TtsController(IPiperTtsService tts) => _tts = tts;
    ...
    public async Task<IActionResult> Synthesize([FromBody] TtsRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var audio = await _tts.SynthesizeAsync(request.Text, request.Voice, request.Speed, cancellationToken);
    ...
    public sealed record TtsRequest(string Text, string Voice = "ff_siwis", double Speed = 1.0);
}
